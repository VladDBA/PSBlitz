...
SELECT CONVERT(VARCHAR(25),CAST([CheckDate] AS DATETIME),120) AS [CheckDate],
	   ...
	   [session_id],
	   [blocking_session_id],
       CAST([query_cost] AS DECIMAL(38,5)) AS [query_cost],
       [status],
       [wait_info],
       [top_session_waits],
       ...
       REPLACE([wait_order],'N/A','') AS [wait_order],
       REPLACE([wait_time_ms],'N/A','') AS [wait_time_ms],
       REPLACE([next_candidate_for_memory_grant],'N/A','') AS [next_candidate_for_memory_grant],
       [target_memory_kb],
...
SELECT CONVERT(VARCHAR(25),[agg].[start_time],120) AS [start_time],
       ...
       [who].[outer_command],
       [who].[query_plan],
       CAST([who].[query_cost] AS DECIMAL(38,5)) AS [query_cost],
       ...
       REPLACE([who].[wait_order],'N/A','') AS [wait_order],
       REPLACE([who].[wait_time_ms],'N/A','') AS [wait_time_ms],
       REPLACE([who].[next_candidate_for_memory_grant],'N/A','') AS [next_candidate_for_memory_grant],
