...
- Execution plans (as .sqlplan files) - from the sample execution plans provided by `sp_BlitzCache @Mode = 0` for missing index suggestions (only on SQL Server 2019)
...
Execution plans file naming convention - `[SortOrder]_[RowNumber].sqlplan` for sp_BlitzCache, and `MissingIndex_[MissingIndexNumber].sqlplan` for sp_BlitzIndex (only available in SQL Server 2019)
