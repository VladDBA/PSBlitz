...
- Execution plans (as .sqlplan files) - from sp_BlitzLock, if any of the execution plans involved in the identified deadlocks are still	in the plan cache at the time of the check
...
Deadlock graphs will be saved in the Deadlocks directory under the output directory.
...
Execution plans file naming convention:
 ...
 - for plans retrieved by sp_BlitzLock - `Deadlock_[RowNumber].sqlplan`.
